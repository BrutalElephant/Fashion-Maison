...
(function(){
 ...
 function init(){const hero=document.querySelector('.heroimg');if(hero&&!hero.dataset.rotating){const frames=['assets/afropop-hero.png','assets/afropop-watch.png','assets/street-sunglasses.png','assets/native-leather.png'];let i=0;hero.dataset.rotating='true';if(!reduce)setInterval(()=>{i=(i+1)%frames.length;hero.classList.add('fm-frame-out');setTimeout(()=>{hero.style.backgroundImage=`url('${frames[i]}')`;hero.classList.remove('fm-frame-out')},420)},5200)}const nodes=document.querySelectorAll('.reveal,.story-feature,.signature,.category-tile,.brand-moment');if(reduce){nodes.forEach(reveal);return}const io=new IntersectionObserver((entries)=>entries.forEach(e=>{if(e.isIntersecting){reveal(e.target);io.unobserve(e.target)}}),{threshold:.14,rootMargin:'0px 0px -8%'});nodes.forEach(n=>io.observe(n));
